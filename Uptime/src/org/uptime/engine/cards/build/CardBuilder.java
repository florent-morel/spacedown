package org.uptime.engine.cards.build;
...
public class CardBuilder {
	...
	public List<Card> buildAvailableCards(Constants.RunMode runMode, Integer numberOfCards) {
		...
		if (cardList == null || Constants.RunMode.HARDCODE.equals(runMode)) {
			cardList = importHardCodedListToDB();
		}
	...
	public List<Card> buildCardListForGame(List<Card> cardList, Integer numberOfCards) {
		...
		if (cardList != null && !cardList.isEmpty() && cardList.size() >= numberOfCards) {
	 ...
	 * @return
	 */
	public List<Card> getRandomCards(List<Card> initialList, int numberOfCards) {
	...
	public List<Card> importHardCodedListToDB() {

		// Backup custom cards
		List<Card> allCustomCards = datasource.getAllCustomCards();

		// Flush current table
		datasource.dropCardTable();
		...
		while (st.hasMoreTokens()) {
			...
			newCard = datasource.createCard(newCard);
			cardList.add(newCard);
		}
